...
const themeInitScript = `(function(){try{var k='savira_display_prefs';var p=JSON.parse(localStorage.getItem(k)||'{}');var r=document.documentElement;var t=localStorage.getItem('token');var d=(p.theme==='dark'||(p.theme!=='light'&&p.theme!=='dark'&&window.matchMedia('(prefers-color-scheme: dark)').matches))&&!!t;if(p.theme||!t)r.dataset.theme=d?'dark':'light';if(p.fontSize)r.dataset.fontSize=p.fontSize;if(p.reducedMotion)r.dataset.reducedMotion='true';if(p.highContrast)r.dataset.highContrast='true';if(p.screenReaderHints===false)r.dataset.screenReaderHints='false';if(p.language)r.lang=p.language;}catch(e){}})();`;

...
function CreateUserModal({ open, onClose, onSave, committees }) {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const response = await internalApiFetch(`/api/users`, {
        method: 'POST',
...
export default function AdminDashboard() {
  ...
  async function handleUpdate(updated) {
    ...
    if (updated._imageFile) {
      ...
      const avatarResponse = await authFetch(`/api/users/${updated.user_id}/avatar?bypass_avatar_cooldown=1`, {
        method: "POST",
        body: avatarForm,
      });
    ...
    const response = await authFetch(`/api/users/${updated.user_id}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },

...
function isWithinCancellationRescheduleWindow(interview) {
  ...
  const timestamp = new Date(cancelledAt).getTime();
...
function requiresRescheduleResponse(interview) {
  return Boolean(interview?.rescheduleRequiresResponse || interview?.reschedule_requires_response);
}
...
function InviteToInterviewModal({ open, onClose, caseData, actorName, showToast, userId, userRole }) {
  ...
  async function handleSend() {
    ...
    try {
      ...
      const interviewRes = await internalApiFetch(`${API_URL}/api/interviews`, {
        method: "POST",
        credentials: "include",
...
function InvitedView({ caseData, interview, onSlotSelected, onAvailabilityRequested, showToast }) {
  ...
  async function handleConfirmSlot(slot) {
    setConfirming(true);
    try {
      const res = await internalApiFetch(`${API_URL}/api/interviews/${interview.id}/select-slot`, {
        method: "PATCH",
        credentials: "include",
        headers: { "Content-Type": "application/json" },
  ...
  async function handleRequestNewSlots(request) {
    ...
    const res = await internalApiFetch(`${API_URL}/api/interviews/${interview.id}/request-new-slots`, {
      method: "PATCH",
      credentials: "include",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
...
function RescheduleReasonModal({ open, onClose, onContinue }) {
  const defaultReason = "I am no longer available at the selected date and time because ";
  const [reason, setReason] = useState(defaultReason);
  ...
  const handleContinue = () => {
    ...
    if (!normalizedReason || normalizedReason === defaultReason.trim()) {
      setError("Please provide a reason before choosing a new slot.");
      return;
    }
    setReason(defaultReason);
    setError("");
  ...
  return (
    <Modal open={open} onClose={handleClose} title="Reason for Rescheduling">
      <FormGroup
        label="Why are you unavailable?"
        required
        hint="Your case officer will see this reason with the updated schedule."
        error={error}
      >
        <FTextarea
          value={reason}
          ...
          placeholder="Briefly explain why the current interview schedule does not work for you."
          maxLength={500}
          error={error}
        />
      </FormGroup>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "1.25rem" }}>
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", flexWrap: "wrap", gap: 8 }}>
        ...
        {onReschedule && !rescheduleRequiresResponse && (
        ...
        {rescheduleRequiresResponse && interview.rescheduleReason && (
          <div className={styles.detailRow}>
            <div>
              ...
              <p className={styles.detailValue}>{interview.rescheduleReason}</p>
            </div>
          </div>
        )}
        {interview.meetingLink && (
...
function RescheduleModal({ interview, mode, userId, onClose, onConfirm }) {
  ...
  useEffect(() => {
    const loadSlots = async () => {
      try {
        const res = await internalApiFetch(`${API_URL}/api/interview_slots?slot_type=case_report&is_available=true&created_by=${userId}`, { credentials: "include" });
        const body = await res.json();
  ...
  async function handleConfirm() {
    ...
    if (mode !== "reopen-selection" && !reason.trim()) {
      setError("Enter a reason for rescheduling.");
      return;
    }
    setSubmitting(true);
    try {
      await onConfirm(interview, selectedSlot, mode, reason.trim());
    } finally {
      setSubmitting(false);
    }
  }
  ...
  return (
    <Modal open onClose={onClose} title={mode === "reopen-selection" ? "Offer New Interview Slots" : "Reschedule Interview"}>
      ...
      {mode !== "reopen-selection" && (
        <FormGroup label="Reason for rescheduling" required>
          <FTextarea
            value={reason}
            onChange={(event) => {
              setReason(event.target.value);
...
function CancelInterviewModal({ interview, onClose, onConfirm }) {
  ...
  async function handleConfirm() {
    if (reason.trim() === defaultReason.trim()) {
  ...
  return (
    <Modal open onClose={onClose} title="Cancel Interview">
      <FormGroup label="Cancellation reason" required error={error}>
        <FTextarea value={reason} onChange={(event) => { setReason(event.target.value); setError(""); }} error={error} />
      </FormGroup>
      <div className={styles.modalFooter}>
        <button className={styles.btnSecondary} onClick={onClose} disabled={submitting}>Keep Interview</button>
...
export default function InterviewTab({ caseData, isStaff, isCaseOfficer, showToast, userId, actorName,
        ...
        setInterviews(
          (json.data || []).map((iv) => {
            ...
            return {
              ...
              rescheduleReason: iv.reschedule_reason || null,
              rescheduleRequiresResponse: Boolean(iv.reschedule_requires_response),
              rescheduleRespondedAt: iv.reschedule_responded_at || null,
  ...
  async function handleSchedule() {
    ...
    try {
      ...
      const caseRes = await internalApiFetch(`${API_URL}/api/case_reports/${caseData.id}`, {
        credentials: "include",
      });
      ...
      const slotRes = await internalApiFetch(`${API_URL}/api/interview_slots`, {
        method: "POST",
        credentials: "include",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
      ...
      const selectRes = await internalApiFetch(`${API_URL}/api/interviews/${interview.interview_id}/select-slot`, {
        method: "PATCH",
        credentials: "include",
        headers: { "Content-Type": "application/json" },
  ...
  async function handleCancel(interviewId, cancellationReason) {
    try {
      const res = await internalApiFetch(`${API_URL}/api/interviews/${interviewId}/cancel`, {
        method: "PATCH",
        credentials: "include",
        headers: { "Content-Type": "application/json" },
  ...
  async function handleComplete(interviewId) {
    ...
    try {
      const res = await internalApiFetch(`${API_URL}/api/interviews/${interviewId}/complete`, {
        method: "PATCH",
        credentials: "include",
      });
  ...
  async function handleStaffRescheduleConfirm(interview, slot, mode, reason = "") {
    try {
      const endpoint = mode === "reopen-selection" ? "reopen-selection" : "reschedule";
      const selectRes = await internalApiFetch(`${API_URL}/api/interviews/${interview.id}/${endpoint}`, {
        method: "PATCH",
        credentials: "include",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(
          ...
          iv.id === interview.id
            ? mode === "reopen-selection"
              ...
              : {
                  ...
                  rescheduleRequiresResponse: true,
                  availabilityRequest: null,
                  availabilityRequested: false,
                }
            : iv
  ...
  async function acceptReschedule(interview) {
    const res = await internalApiFetch(`${API_URL}/api/interviews/${interview.id}/accept-reschedule`, {
      method: "PATCH",
      credentials: "include",
      headers: { "Content-Type": "application/json" },
    });
    const body = await res.json().catch(() => ({}));
    if (!res.ok) {
      showToast?.(body.error || "Failed to accept the rescheduled interview.", "error");
      return;
    }
    ...
    showToast?.("Rescheduled interview accepted.");
  }
      ...
      return (
        <>
          <ScheduledView
            ...
            rescheduleRequiresResponse={rescheduleRequiresResponse}
            onAcceptReschedule={() => acceptReschedule(activeInterview)}
            onCancelInterview={() => setCancelInterview(activeInterview)}
          ...
          />
          {cancelInterview?.id === activeInterview.id && (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
          {interviews.map((iv) => {
            const status = iv.interviewStatus || (
              iv.status
                ? iv.status.charAt(0).toUpperCase() + iv.status.slice(1).toLowerCase()
                : "Scheduled"
            );
            ...
            const canRescheduleCancelled =
              (isStaff || isCaseOfficer) &&
              isCancelled &&
              isWithinCancellationRescheduleWindow(iv);
            ...
            >
              ...
              {iv.availabilityRequest && (
                <div style={{ padding: "0.8rem 0.9rem", borderRadius: 8, border: "1px solid #f5c26b", background: "#fff8e6", color: "#7c4a03" }}>

    ...
    async function fetchScores() {
      try {
        ...
        const [essayRes, interviewRes] = await Promise.all([
          internalApiFetch(`${API}/api/volunteer_applications/${appData.id}/essay_evaluation`),
          internalApiFetch(`${API}/api/volunteer_applications/${appData.id}/interview_evaluation`),
        ]);
...
function ApplicationEvaluationTab({ appData, isAdmin, canEdit, onUpdateStatus }) {
  ...
  useEffect(() => {
    ...
    async function fetchQuant() {
      try {
        const res = await internalApiFetch(`${API_URL}/api/volunteer_applications/${appData.id}/scores`);
        if (res.ok) {
  ...
  useEffect(() => {
    ...
    async function fetchNlp() {
      try {
        const res = await internalApiFetch(`${API_URL}/api/volunteer_applications/${appData.id}/nlp`);
        if (!res.ok) return;
        const json = await res.json();
        const d = json.data || json;
...
export default function ViewApplication() {
  ...
  useEffect(() => {
    ...
    async function fetchApp() {
      try {
        const API = API_URL;
        const res = await internalApiFetch(`${API}/api/volunteer_applications/${appId}`);
        if (!res.ok) {

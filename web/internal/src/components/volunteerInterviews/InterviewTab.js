  ...
  async function handleSend() {
    ...
    try {
      ...
      if (!intervieweeUserId) {
        const appRes = await internalApiFetch(`${API_URL}/api/volunteer_applications/${appData.id}`, {
          credentials: "include",
        });
...
export default function InterviewTab({ appData, isStaff, isApplicationOfficer, showToast, userId, actorName,
  ...
  useEffect(() => {
    if (appData.applicantUserId) return;
    const fetchApplicantUserId = async () => {
      try {
                const res = await internalApiFetch(`${API_URL}/api/volunteer_applications/${appData.id}`, {
          credentials: "include",
        });
        if (!res.ok) return;
        const json = await res.json();
  ...
  async function handleStaffRescheduleConfirm(updated) {
    ...
    try {
      ...
      const selectRes = await internalApiFetch(`${API_URL}/api/interviews/${updated.id}/select-slot`, {
        method: "PATCH",
        credentials: "include",
        headers: { "Content-Type": "application/json" },

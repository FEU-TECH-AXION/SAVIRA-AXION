...
export async function fetchNotifications({ force = false } = {}) {
  ...
  const response = await internalApiFetch('/api/notifications');
...
export async function markAllRead() {
  ...
  try {
    await internalApiFetch('/api/notifications/read-all', { method: 'PATCH' });
  } catch (error) {
    emit(previous);
    throw error;
  }
...
export async function dismissNotification(id) {
  ...
  try {
    const response = await internalApiFetch(`/api/notifications/${id}`, { method: 'DELETE' });
